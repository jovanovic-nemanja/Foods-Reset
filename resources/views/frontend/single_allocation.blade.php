        ...
        <section role="register-page" class="py-5">
            <div class="container">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class='row'>
                            <div class='col-sm-10'><h6>Allocation Detail</h6></div>
                        </div>
                        <table class="table table-striped table-bordered">
                            ...
                            <tr>
                                <td>{{$allocation->id}}</td>
								<?php
								$buyer_post = App\Models\BuyerPost::where( 'buyer_post_id', $allocation->buyer_post_id )->with( 'product' )->first();
								$supplier_post = App\Models\SupplierPost::where( 'supplier_post_id', $allocation->supplier_post_id )->first();
                                ...
                                <td>{{number_format($allocation->allocation)}}</td>
                                <td>{{number_format($allocation->reallocation)}}</td>
                                <td class="rating">
                                    @if($allocation->status == 'accept')
                                        <a href="javascript:void(0);" class="btn" data-toggle="modal"
                                           data-target="#rating_modal">
                                            @if($allocation->rating > 0)
                                                @for($i=1; $i<= $allocation->rating; $i++)
                                ...
                                @if(Auth::user()->user_role == 3)
                                    <td><a style='color:blue;' target='_blank'
                                           href="{{url('/')}}/addbank/detail/{{$allocation->supplier_post_id}}/{{$allocation->id}}">Add
                                            Bank Detail</a></td>
                                @endif
                                <td>
                                    <select class='form-control allocation_status allocation_status_{{$allocation->id}}'
                                            id='{{$allocation->id}}' @if(Auth::user()->user_role == 2) disabled
                                            readonly @endif>
                                        <option {{$allocation->status == 'pending'?'selected="selected"':''}} value='pending'>
                                            Pending
                                        </option>
                                        <option {{$allocation->status == 'accept'?'selected="selected"':''}} value='accept'>
                                            Accept
                                        </option>
                                        <option {{$allocation->status == 'reject'?'selected="selected"':''}} value='reject'>
                                            Reject
                                        </option>
                                    </select>
                                </td>
...
//              $(".allocation_status_"+allocation_id).val(last_val);
                }
...
<div class="modal fade" id="rating_modal" tabindex="-1" role="dialog" style='margin-top: 20px;'
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form class="" role="form" method="POST" action="{{url('/')}}/addrating/{{$allocation->id}}">
                {{ csrf_field() }}
                <div class="modal-body">
                    <div class="form-group" style="font-size:13px;">
                        <label>Rating</label>
                        <select id="" class="custom-select form-control" name="rating">
                            <option {{$allocation->rating == '1'?'selected="selected"':''}} value="1">1 Star</option>
                            <option {{$allocation->rating == '2'?'selected="selected"':''}} value="2">2 Star</option>
                            <option {{$allocation->rating == '3'?'selected="selected"':''}} value="3">3 Star</option>
                            <option {{$allocation->rating == '4'?'selected="selected"':''}} value="4">4 Star</option>
                            <option {{$allocation->rating == '5'?'selected="selected"':''}} value="5">5 Star</option>

                        </select>
                    </div>
                    <div class="form-group" style="font-size:13px;">
                        <label>Comment</label>
                        <textarea class="form-control" name="comments" value="" id='' placeholder="Type Your Comments"
                                  type="text">{{$allocation->comments}}</textarea>
                    </div>
                </div>
